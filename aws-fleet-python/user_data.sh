...
nohup python3 -m http.server 80 > file.txt 2>&1 & | tee -a $output_file
mkdir -p $home_dir/docker_volume
run docker run --network=host -v $docker_vol:/dev/shm -d --name graphstorm mohammadzainabbas/graphstorm:local | tee -a $output_file
cd - || exit